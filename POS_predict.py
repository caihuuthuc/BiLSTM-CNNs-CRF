...
from POS_datahelper import readFileTSV, load_from_file, word_2_indices_per_char, get_word_from_idx, word_indices_to_char_indices, next_lr
...
if __name__ == '__main__':
    ...
    with tf.Session(config = config) as sess:
        ...
        saver = tf.train.import_meta_graph('./saved_model/pos/ckpt.meta')
    
        with tf.device("/device:gpu:0"):
            print('Prepare data: Start')
            saver.restore(sess, tf.train.latest_checkpoint('./saved_model/pos/'))
            ...
            tsvfile = open('POS_predict.tsv', 'w')
