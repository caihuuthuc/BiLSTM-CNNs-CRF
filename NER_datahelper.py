...
def get_labels_template(labels, tagging):
    labels_template = []
    labels_template.append('O')
    labels_template.append('PAD')
    for label in ['PER', 'ORG', 'LOC', 'MISC']:
        for tag in list(set(tagging) - set('O')):
            t = tag+'-'+label
            if t not in labels_template:
                labels_template.append(t)
...
if __name__ == '__main__':
    def preprocess():
        ...
        labels_template = get_labels_template(labels, tagging)
        ...
        write_to_file(data, "./data_feed_model_NER.shlv")
