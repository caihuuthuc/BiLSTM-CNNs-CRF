...
from Task1_datahelper import load_from_file, word_2_indices_per_char, batch_iter, get_word_from_idx, word_indices_to_char_indices, next_lr
...
def get_variation_word_embedding(lookup_table, type='nonstatic'):
    if type == 'nonstatic':
        return tf.Variable(name='word-embedding', initial_value=lookup_table, dtype=tf.float32, trainable=True)
    elif type == 'static':
        return tf.Variable(name='word-embedding', initial_value=lookup_table, dtype=tf.float32, trainable=False)
    elif type == 'multichannel':
        W_embedding_nonstatic = tf.Variable(name='nonstatic-word-embedding', initial_value=lookup_table, dtype=tf.float32, trainable=True)
        W_embedding_static = tf.Variable(name='static-word-embedding', initial_value=lookup_table, dtype=tf.float32, trainable=False)
        W_embedding = tf.concat([W_embedding_static, W_embedding_nonstatic], axis=-1, name='word-embedding')
        return W_embedding
...
with tf.Session(config = config) as sess:
    ...
    saver.save(sess, './saved_model/ner/ckpt')
